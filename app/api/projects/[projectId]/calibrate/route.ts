...
export async function POST(req: NextRequest, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  let memoryResult = await pool.query(`SELECT * FROM project_memory WHERE project_id = $1 AND user_id = $2`, [projectId, userId]).catch(() => ({ rows: [] }));
...
Do not re-explain the user's general profile. Focus only on project-specific calibration.`;
  ...
  await pool.query(`
    ...
    ON CONFLICT (project_id, user_id) DO UPDATE SET
      ...
      last_calibrated = NOW(),
      updated_at = NOW()
  `, [projectId, userId, JSON.stringify(snapshot), JSON.stringify([snapshot])]).catch(async () => {
  ...
  if (accuracy_rating) {
    await pool.query(`UPDATE projects SET confidence = $1, updated_at = NOW() WHERE id = $2`, [accuracy_rating, projectId]);
  }
